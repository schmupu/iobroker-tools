...
// createNow must be boolean and not a function
function setOrUpdateObjectAsync(id, obj, obtainCustomFields, value, stateChangeCallback, createNow) {
...
function setOrUpdateObjectAsync(id, obj, obtainCustomFields, value, stateChangeCallback, createNow) {
    return new Promise((resolve, reject) => {
        ...
        // if createNow is set than setOrUpdateObject is called with a pseudo callback function
        // if value set we have to create a callback function for adapter.setState
        if (createNow === true || (value !== undefined && value !== null)) {
            createNow = () => { resolve() };
            setOrUpdateObject(id, obj, obtainCustomFields, value, stateChangeCallback, createNow);
        } else {
            setOrUpdateObject(id, obj, obtainCustomFields, value, stateChangeCallback, createNow);
            resolve();
        }
...
function deleteObject(id) {
    const obj = adapterObjects[id];
    if (obj && obj.type) {
        let adapterObjectsDel = [];
        if (obj.type !== 'state') {
            Object.keys(adapterObjects).forEach((objId) => {
                if (objId.startsWith(id + '.')) {
                    adapterObjectsDel.push(ojbId);
                }
            });

        }
        adapterObjectsDel.push(id);
        function delObject(callback) {
            let objId = adapterObjectsDel.pop();
            if (ojbId) {
                adapter.delObject(objId, (err) => {
                    err = err ? ' (' + err + ')' : '';
                    adapter.log.info(adapterObjects[objId].type + ' ' + objId + ' deleted' + err);
                    delete adapterObjectsDel[objId];
                    if (!err) {
                        delete adapterObjects[objId];
                    }
                    delObject(callback);
...
// const deleteObjectAsync = util.promisify(deleteObject);
function deleteObjectAsync(id) {
    return new Promise((resolve, reject) => {
        deleteObject(id, () => { resolve(); })
    });
}
...
// const processObjectQueueAsync = util.promisify(processObjectQueue);
function processObjectQueueAsync() {
    return new Promise((resolve, reject => {
        processObjectQueue(() => { resolve(); })
    }))
}
...
// const loadExistingObjectsAsync = util.promisify(loadExistingObjects);
function loadExistingObjectsAsync() {
    return new Promise((resolve, reject => {
        loadExistingObjects(() => { resolve(); })
    }))
}
...
module.exports = {
    ...
    setOrUpdateObjectAsync: setOrUpdateObjectAsync,
    deleteObject: deleteObject,
    deleteObjectAsync: deleteObjectAsync,
    processObjectQueue: processObjectQueue,
    processObjectQueueAsync: processObjectQueueAsync,
    loadExistingObjects: loadExistingObjects,
    loadExistingObjectsAsync: loadExistingObjectsAsync,
    getObject: getObject,
